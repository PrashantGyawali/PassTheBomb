...
(async function()
{
    ...
    let selfRadius=28;
    let resolveSelfId;
    let selfIdReturner=new Promise((resolve,reject)=>{resolveSelfId=resolve}),selfId;
    let otherplayersId={};
    let bomberId;
    ...
    ws.onmessage=function(message){
        ...
        if(messagedata.type=="position")
        {
            ...
            setTimeout(()=>{
            cursor.style.transition = `transform 1s linear`;
            
            let angle=-(Math.atan((cursor.getBoundingClientRect().y-messagedata.y)/(cursor.getBoundingClientRect().x-messagedata.x))*180/3.1415 +(cursor.getBoundingClientRect().y-messagedata.y)>0?180:-180)*0.2;
            otherplayersId[messagedata.sender]=messagedata.sender;
            cursor.style.transform = `translate(${messagedata.x*resolutionRatio}px, ${messagedata.y*resolutionRatio}px) translate(-50%,-50%) rotate(${angle}deg)`;},1)

            timers[messagedata.sender]=setTimeout(()=>{
            cursor.style.transition = `transform 0.13s linear`;
            cursor.style.transform = `translate(${messagedata.x*resolutionRatio}px, ${messagedata.y*resolutionRatio}px) translate(-50%,-50%)`;},850)
        }
